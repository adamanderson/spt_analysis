...
fields = ['ra5hdec-24.5', 'ra5hdec-31.5'] #, 'ra5hdec-38.5', 'ra5hdec-45.5', 'ra5hdec-52.5', 'ra5hdec-59.5']
...
for field in fields:
    ...
    frac_cal_all_middle_error_10 = {wafer: {band: [] for band in band_list} for wafer in wafer_list}
    median_middle_error = {wafer: {band: [] for band in band_list} for wafer in wafer_list}
    ...
    for jobsid, oi_field in enumerate(list(obsids_middle.keys())):
        ...
        cal_response_middle_error = (((cal_response_top + cal_response_bottom)/2) - cal_response_middle) / cal_response_middle
        ...
        for wafer in wafer_list:
            ...
            for band in band_list:
                ...
                cal_error_plot = cal_response_middle_error[np.isfinite(cal_response_middle_error) & \
                                                           (bands == band) & (wafer_selection)]
                good_bolos_frac = len(cal_error_plot[np.abs(cal_error_plot)<0.1]) / len(cal_error_plot)
                good_bolos_median = np.median(np.abs(cal_error_plot))
                frac_cal_all_middle_error_10[wafer][band].append(good_bolos_frac)
                median_middle_error[wafer][band].append(good_bolos_median)
                plt.hist(cal_error_plot,
                         bins=np.linspace(-1,1,51), color=band_colors[band],
                         histtype='step', label='{} GHz ({:.1f}% with less than +/-10% error)'.format(band, 100*good_bolos_frac))
            plt.title('{}: {}'.format(field, oi_field))
            plt.legend()
            plt.xlabel('"error" := ((top + bottom) / 2 - middle) / middle')
            plt.tight_layout()
            plt.savefig('figures/cal_middle_error_{}_{}_{}.png'.format(field, jobsid, wafer), dpi=200)
            plt.close()

    for wafer in wafer_list:
        plt.figure()
        for band in band_list:
            cal_error = frac_cal_all_middle_error_10[wafer][band]
            plt.plot(np.arange(len(cal_error)), cal_error,
                     color=band_colors[band], linestyle='-', marker='o',
                     label='{} GHz'.format(band))
        plt.legend()
        plt.title('"error" := ((top + bottom) / 2 - middle) / middle\n{}, {}'.format(field, wafer))
        plt.xlabel('observation index')
        plt.ylabel('fraction of bolometers with abs(error) < 10%')
        plt.ylim([0,1])
        plt.tight_layout()
        plt.savefig('figures/cal_error_10p_{}_{}.png'.format(field, wafer))
        plt.close()


        plt.figure()
        for band in band_list:
            cal_error = median_middle_error[wafer][band]
            plt.plot(np.arange(len(cal_error)), cal_error,
                     color=band_colors[band], linestyle='-', marker='o',
                     label='{} GHz'.format(band))
        plt.legend()
        plt.title('"error" := ((top + bottom) / 2 - middle) / middle\n{}, {}'.format(field, wafer))
        plt.xlabel('observation index')
        plt.ylabel('median of abs(error)')
        plt.ylim([0,1])
        plt.tight_layout()
        plt.savefig('figures/cal_error_median_{}_{}.png'.format(field, wafer))
        plt.close()
        ...
        if False:
            for wafer in wafer_list:
                ...
                plt.tight_layout()
                plt.savefig('figures/mid_bottom_{}_{}_{}.png'.format(field, jobsid, wafer), dpi=200)
                plt.close()
                ...
                plt.xlabel('fractional change in cal. response')
                plt.tight_layout()
                plt.savefig('figures/top_mid_{}_{}_{}.png'.format(field, jobsid, wafer), dpi=200)
                plt.close()
                ...
                plt.xlabel('fractional change in cal. response')
                plt.tight_layout()
                plt.savefig('figures/top_bottom_{}_{}_{}.png'.format(field, jobsid, wafer), dpi=200)
                plt.close()
