...
for pairname in d[3]['PairDiffTimestreams'].keys():
    ...
    ts_sum = np.array(d[3]['PairSumTimestreams'][pairname]) / np.sqrt(2.)
    ff, psd_diff = periodogram(ts_diff, fs=152.5)
    ff, psd_sum = periodogram(ts_sum, fs=152.5)
    wafer_psds_sum[wafer][band][pairname] = np.sqrt(psd_sum) / (1. / np.sqrt(core.G3Units.Hz)) * 1e6
    wafer_psds_diff[wafer][band][pairname] = np.sqrt(psd_diff) / (1. / np.sqrt(core.G3Units.Hz)) * 1e6
...
for wafer in wafer_psds_sum:
    for band in [90., 150., 220.]:
        ...
        plt.legend()
        plt.ylim([1e2, 1e8])
        plt.savefig('{}_{}_mean_psd.png'.format(wafer, int(band)), dpi=200)
        plt.close()
        ...
        plt.legend()
        plt.ylim([1e2, 1e8])
        plt.savefig('{}_{}_median_psd.png'.format(wafer, int(band)), dpi=200)
        plt.close()
