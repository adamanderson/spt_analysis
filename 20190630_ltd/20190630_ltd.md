...
for jband, band in enumerate([90, 150, 220]):
    ...
    _ = plt.hist(1e3*tau_plot1, bins=np.linspace(0, 12, 26),
...
fbias_fnal = fbias[np.isfinite(Rn)]
Rp_fnal = Rp[np.isfinite(Rn)]
Rn_fnal = Rn[np.isfinite(Rn)]
```
...
```python
psat_315mK = {90:[], 150:[], 220:[]}
for bolo in d_psat.keys():
    band = int(bolo.split('/')[1].split('.')[1])
    psat_315mK[band].append( d_psat[bolo]['k_IV'] * \
                            (d_psat[bolo]['Tc_IV']**d_psat[bolo]['n_IV'] - \
                             0.315**d_psat[bolo]['n_IV'])*1e12 )
```

```python
for band in psat_315mK:
    _ = plt.hist(psat_315mK[band], bins=np.linspace(0, 20, 41), alpha=0.5)
```

Next let's get measurements of the optical Psat from all of June 2019 for typical winter observing conditions.

```python
joule_path = '/spt/user/adama/20190630_ltd/downsampled/'
fnames = glob(os.path.join(joule_path, '*'))
fnames = np.sort(fnames)
...
for fname in fnames:
    ...
    if len(djoule) == 2: # should include calibration frame and frame with average power
        psat_pole_315mK[obsid] = {90:[], 150:[], 220:[]}
        for bolo in boloprops.keys():
            if boloprops[bolo].wafer_id == 'w206' and \
               bolo in djoule[1]['AvgPower'].keys() and \
               boloprops[bolo].band/core.G3Units.GHz in psat_pole_315mK[obsid].keys():
                psat_pole_315mK[obsid][boloprops[bolo].band/core.G3Units.GHz]\
                    .append(djoule[1]['AvgPower'][bolo] / (1e-12*core.G3Units.watt))

        for band in psat_pole_315mK[obsid]:
            psat_pole_315mK[obsid][band] = np.array(psat_pole_315mK[obsid][band])
```
