...
\begin{equation}
P \rightarrow \frac{R_b^2 + (2\pi f L_b)^2}{R_b^2} P
\end{equation}
...
\begin{equation}
P_{sat}(T_{cl}) = P_{sat}^{dark}(T_{stage}(0)) - \left< C \right> T_{cl} - \eta \frac{R_b^2}{R_b^2 + (2\pi f L_b)^2} \int \frac{h \nu}{\exp{h\nu / kT} - 1} R_{ant}(\nu) R_{filt}(\nu) ~d\nu,
\end{equation}
...
hwm_fname = '/Users/adama/spt/software/hardware_maps/fnal/run33/hwm.yaml'
...
filter_data = [np.array([0, 1e12]), np.array([0.95, 0.95])]
```
...
ax[0].legend()
ax[0].set_ylim([0,1.2])
ax[0].set_xlabel('bias frequency [MHz]')
...
ax[1].set_title('90 GHz\nmean eff. = {:.2f}'.format(np.mean(eff)))
ax[1].grid()
