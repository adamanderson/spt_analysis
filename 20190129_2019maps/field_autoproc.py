...
parser.add_argument('-x', '--xlen', default = 50, type=float,
                   help='map width [deg]')
parser.add_argument('-y', '--ylen', default = 30, type=float,
                   help='map height [deg]')
...
# # Fix bolometer polarization angles
# def fix_pol_angles(fr):
#     if 'BolometerProperties' not in fr:
#         return
#     bp = deepcopy(fr.pop('BolometerProperties'))
#     for bolo in bp.keys():
#         if hasattr(bp[bolo],'pol_angle'):
#             bp[bolo].pol_angle *= -1
#     fr['BolometerProperties'] = bp
# pipe.Add(fix_pol_angles)
...
# This will be for flagging anomalously large higher-freq power,
# due to e.g. oscillating bolos. Primarily for 48* observations 
pipe.Add(std_processing.weighting.AddPSDWeights,
         input = 'CalTimestreams', output = 'HighFWeights',
         low_f = 8 * core.G3Units.Hz,
         high_f = 12 * core.G3Units.Hz, store_power = True)
# Flag bolos failing my high-f power  cut
pipe.Add(timestreamflagging.flaggingutils.FlagBadG3MapValue,
         m_key = 'PsdPower', max_val = 200, flag_key='Flags',
         flag_reason = 'HighFPower')
...
# If flagging has removed all bolos, drop the frame.
# This prevents the common-mode filter from crashing.
def ts_check(fr, ts_key):
    if fr.type == core.G3FrameType.Scan:
        if ts_key not in fr or len(fr[ts_key])==0:
            return False
...
pipe.Add(timestreamflagging.flaggingutils.SigmaclipFlagGroupG3MapValue,
         m_key = 'TodWeights', low = 2.5, high = 2.5, per_band = True,
