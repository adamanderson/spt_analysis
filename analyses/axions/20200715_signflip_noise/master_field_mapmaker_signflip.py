...
if not args.sim:
    ...
    if args.sign_flip_noise:
        pipe.Add(axiontod.FlipScanSigns(seed=args.sign_flip_seed,
                                        input_key_i=raw_ts_key_i,
                                        input_key_q=raw_ts_key_q,
                                        output_key_i=raw_ts_key_i + 'Flipped',
                                        output_key_q=raw_ts_key_q + 'Flipped',
                                        az_key='OnlineBoresightAz'))
        pipe.Add(core.Delete, [raw_ts_key_i, raw_ts_key_q])
        raw_ts_key = raw_ts_key + 'Flipped'
        raw_ts_key_i = raw_ts_key_i + 'Flipped'
        raw_ts_key_q = raw_ts_key_q + 'Flipped'
    ...
    # Multiply scans by +/- in order to cancel CMB and create a noise simulation.
    # In this block, this is mostly intended to test the cancellation from the 
    # sign-flipping.
    if args.sign_flip_noise:
        pipe.Add(axiontod.FlipScanSigns(seed=0,
                                        input_key=pointing_ts_key,
                                        output_key=pointing_ts_key + 'Flipped',
                                        az_key=pmodel+'BoresightAz'))
        pointing_ts_key = pointing_ts_key + 'Flipped'
        pre_poly_ts_key = pointing_ts_key
