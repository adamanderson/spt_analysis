...
config.add_argument('--split-by-wafer', default = False, action = 'store_true',
                    help = ('Split data from different wafers into '
                            'different maps.'))
config.add_argument('--complete-pol-pairs', default = False,
                    action = 'store_true',
                    help = ('Restrict to map to detectors with complete '
                            'polarization pairs.'))
config.add_argument('--nuclear-gain-matching', default = False,
                    action = 'store_true',
                    help = ('Use scan-by-scan nuclear gain-matching method '
                            'which adjusts power in the pair-differenced '
                            'timestream in a user-specified frequency band.'))
config.add_argument('--nuclear-low-freq', type = float, default = 0.1,
                    help = ('Lower limit of the frequency range, in Hz, over '
                            'which the power of the pair-differenced TOD is '
                            'minimized when calculating gain-matching '
                            'coefficients.'))
config.add_argument('--nuclear-high-freq', type = float, default = 1.0,
                    help = ('Upper limit of the frequency range, in Hz, over '
                            'which the power of the pair-differenced TOD is '
                            'minimized when calculating gain-matching '
                            'coefficients.'))
...
def SimStub(fr, ts_key, valid_ids_key = 'valid_ids',
    ...
    if args.nuclear_gain_matching:
        to_keep += ['GainMatchingCoeffs']
...
if not args.sim:
    ...
    pipe.Add(std_processing.flagsegments.FieldFlaggingPostKcmbConversion,
             ...
             filter_mask_key = 'FilterMask', ps_mask_id = ps_map_id,
...
pre_map_ts_key = 'PolyFilteredTimestreams'
...
if args.complete_pol_pairs:
    # -------------------------------------------------------------------------
    # Flag detectors with incomplete polarization pairs
    # -------------------------------------------------------------------------
    pipe.Add(timestreamflagging.FlagIncompletePixelPairs,
             flag_key = 'Flags', ts_key = pre_map_ts_key,
             flag_reason = 'IncompletePixelPair')

if args.nuclear_gain_matching:
    # -------------------------------------------------------------------------
    # Calculate gain-matching coefficients by scan
    # -------------------------------------------------------------------------
    pipe.Add(std_processing.gainmatching.match_gains,
             ts_key = pre_map_ts_key,
             gain_match_key = 'GainMatchingCoeffs',
             freq_range = [args.nuclear_low_freq, args.nuclear_high_freq],
             manual_gain_factor = None)
    pipe.Add(std_processing.gainmatching.apply_gain_matching,
             ts_key = pre_map_ts_key,
             gain_match_key = 'GainMatchingCoeffs',
             out_key = 'GainMatchedTimestreams')
    pre_map_ts_key = 'GainMatchedTimestreams'
    
# -----------------------------------------------------------------------------
# Remove flagged bolos
# -----------------------------------------------------------------------------
pipe.Add(timestreamflagging.RemoveFlagged, 
         input_ts_key = pre_map_ts_key,
         input_flag_key = 'Flags',
         output_ts_key = 'DeflaggedTimestreams')
pipe.Add(core.Delete, keys=[pre_map_ts_key])
